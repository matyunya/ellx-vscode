...
function errorLogger(req: any, res: any, next: any) {
...
export default async (options: FSOptions) => {
  ...
  const auth =
    ...
    (req: FSRequest, res: FSResponse, next: NextFunction) => {
  ...
  try {
    ...
    // @ts-ignore
    app
      .use(logger, json(), helpers, cors())
      // @ts-ignore
      .use("/resource", auth(serveFiles(options.root)))
      // @ts-ignore
      .get("/identity", (_: FSRequest, res: FSResponse) =>
        res.end(options.identity)
      );
    ...
    ws.once("close", () => {
      console.log("trying to close parent server");
      server.close();
    });
